...
export class SupabaseApiClient {
  ...
  // Admin Analytics (for backward compatibility)
  async getAdminAnalytics(timeframe = '30d') {
    return this.request(`/admin/analytics?timeframe=${timeframe}`);
  }

  // Additional admin method aliases for backward compatibility
  ...
  async getAdminOrders(filters = {}) {
    return this.orders.getAll(filters);
  }

  async updateOrderStatus(id, status, trackingNumber = null) {
    return this.orders.update(id, { status, tracking_number: trackingNumber });
  }

  async getAdminCustomers(filters = {}) {
    return this.customers.getAll(filters);
  }
