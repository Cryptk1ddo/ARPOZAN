...
export default function CartDrawer({ open, onClose }) {
  ...
  return (
    <div className={`fixed inset-0 z-50 ${open ? '' : 'pointer-events-none'}`}>
      ...
      {/* Drawer */}
      <aside
        className={`cart-drawer absolute right-0 top-0 h-full w-full max-w-md flex flex-col transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
        aria-hidden={!open}
      >
          ...
          <div className="flex-grow space-y-4 overflow-y-auto">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center gap-4 bg-white/2 rounded-xl p-3"
                >
                  <Image
